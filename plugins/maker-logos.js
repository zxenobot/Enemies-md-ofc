...
if (!effect) throw info2 + map2 + info3
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { return })
   if (typeof res == 'number') throw res == -1 ? `riprova` : `uso del comando: ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}`;
  await conn.sendMessage(m.chat, {image: {url: res.image}, caption: null}, {quoted: m});  
};
handler.help = ['logos'];
handler.tags = ['nulis'];
